...
data_NT.to_csv("1-Obtaining-DEGs-for-HB/Study_Variability/NT_deseq.csv", index=False)

# T
data_T = pd.concat([StudyA_T, StudyB_T], axis=0)
data_T.to_csv("1-Obtaining-DEGs-for-HB/Study_Variability/T_deseq.csv", index=False)

# ONLY DEGS

# Import DEGs

degs = pd.read_csv('1-Obtaining-DEGs-for-HB/DEGs_HB/HB_db_DEG.csv',sep=',')
...
# NT
data_NT = pd.concat([StudyA_NT, StudyB_NT], axis=0)
data_NT.to_csv("1-Obtaining-DEGs-for-HB/Study_Variability/output/NT_deseq_DEGs.csv", index=False)

# T
data_T = pd.concat([StudyA_T, StudyB_T], axis=0)
data_T.to_csv("1-Obtaining-DEGs-for-HB/Study_Variability/output/T_deseq_DEGs.csv", index=False)