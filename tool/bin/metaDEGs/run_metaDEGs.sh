...
do
    ...
    do
        if [ "$Study1" != "$Study2" ]; then
            ...
            Rscript scripts/wasserstein.R $waddR_pvaladj $waddR_permnum > output/means/wass_out.txt 2> /dev/null
...
mkdir -p output; rm -r -f output/*; mkdir -p output/metaDEGs; mv bin/metaDEGs/output/* output/metaDEGs; rm -r -f bin/metaDEGs/src; rm -f bin/metaDEGs/sample_names.txt; rm -r -f bin/metaDEGs/output