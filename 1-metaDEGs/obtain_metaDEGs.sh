...
do
    ...
    if [ -f "$FILE" ]; then
        echo "      >> ${sid} files processed correctly from GREIN"
    else 
...
do
    ...
    echo "      >> $sid DEGs list obtained correctly"
done

echo '3/3 - Obtaining metaDEGs'
Rscript scripts/obtain_ranks.R >& /dev/null
Rscript scripts/table_to_list.R  >& /dev/null

rm -r -f src/tmp # Clean folders
