...
num_rands = pd.read_csv('2-HB-DEGs-on-Liver-Communities/2_Communities/Benchmark/output/temp/output_num_randomizations.txt',header=None, names=["num_rands"])
num_com = pd.read_csv('2-HB-DEGs-on-Liver-Communities/2_Communities/Benchmark/output/temp/output_num_communities.txt',header=None, names=["num_communities"])
avg_size = pd.read_csv('2-HB-DEGs-on-Liver-Communities/2_Communities/Benchmark/output/temp/output_avg_com_size.txt',header=None, names=["avg_community_size"])
...
data_diff.replace([np.inf, -np.inf], 0, inplace=True)

result_serie = data_diff['result'].squeeze()
if result_serie.isnull().all() == True:
    data_diff['result'] = data_diff['result'].fillna(0)
